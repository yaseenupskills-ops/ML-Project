...
class FeatureEngineer:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        # self.fps is the *effective* rate of the processed keypoint sequence:
        # pose_extraction.py only yields every `pose.frame_stride`-th frame, so
        # consecutive keypoints are frame_stride/raw_fps seconds apart, not 1/raw_fps.
        frame_stride = self.config.get('pose', {}).get('frame_stride', 1)
        self.fps = feat_config['fps'] / frame_stride
    ...
    def compute_torso_orientation(self, keypoints: np.ndarray) -> float:
        ...
        if len(points) >= 2:
            # Use PCA to find principal axis (SVD on the (n_points, 2) matrix,
            # not its transpose, so the principal axis stays in x/y space)
            centered = points - np.mean(points, axis=0)
            _, _, vt = np.linalg.svd(centered, full_matrices=False)
            principal_axis = vt[0]  # First principal component

            # Angle from vertical (y-axis): 0 = upright, 90 = lying flat
            angle = np.arctan2(abs(principal_axis[0]), abs(principal_axis[1]))
