...
@unittest.skipUnless(
    ...
    f"MediaPipe model asset not present at {POSE_MODEL_PATH} (download separately, not checked into git)",
)
...
class TestDecisionLogic(unittest.TestCase):
    ...
    def test_majority_vote(self):
        ...
        # Enough windows above medium threshold; tier reflects the average
        # confidence of the voting windows, not a majority of the tiers
        probs = [0.9, 0.9, 0.4]  # Two above high threshold, avg = 0.733
        is_fall, conf, tier = self.dec_logic.majority_vote(probs, [0,1,2])
