...
def extract_keypoints_from_video(video_path: str, config_path: str = "config.yaml") -> List[np.ndarray]:
    """Run PoseExtractor over a video, dropping frames with no detected pose."""
    extractor = PoseExtractor(config_path)
    return [kp for _, kp in extractor.process_video(video_path) if kp is not None]