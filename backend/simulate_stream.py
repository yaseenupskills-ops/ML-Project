...
class FallDetectionSimulator:
    ...
    def run_live_detection(
        ...
        frame_count = 0
        keypoint_buffer = []
        # Debounce: the keypoint buffer is a sliding window, so a real fall
        # produces a fall-classified window on nearly every frame for its
        # duration. Without a cooldown, each of those spawns its own grace
        # period + alert. One grace period per timeout window is enough.
        grace_cooldown_sec = self.config.get('grace_period', {}).get('timeout_sec', 20)
        last_event_time = -grace_cooldown_sec
            ...
            while True:
                ...
                try:
                    ...
                    for event in fall_events:
                        ...
                        now = time.time()
                        if now - last_event_time < grace_cooldown_sec:
                            continue  # already in an active grace-period cooldown
                        last_event_time = now
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "--camera":
        ...
        print(f"Live detection complete: {result['stats']}")
        ...
        print(f"Video detection complete: {result['stats']}")
